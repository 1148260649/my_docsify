...
![图片](../../img/java/并发60问/并行跟并发区别.png "并行跟并发区别")

**就好像我们去食堂打饭，并行就是我们在多个窗口排队，几个阿姨同时打菜；并发就是我们挤在一个窗口，阿姨给这个打一勺，又手忙脚乱地给那个打一勺。**

![图片](../../img/java/并发60问/并行并发和食堂打饭.png "并行并发和食堂打饭")
...
![图片](../../img/java/并发60问/程序进程线程关系.png "程序进程线程关系")
...
![图片](../../img/java/并发60问/线程创建三种方式.png "线程创建三种方式")
...
![图片](../../img/java/并发60问/Java线程状态变化.png "Java线程状态变化")

## 7.什么是线程上下文切换？

使用多线程的目的是为了充分利用CPU，但是我们知道，并发其实是一个CPU来应付多个线程。

![图片](../../img/java/并发60问/线程切换.png "线程切换")

为了让用户感觉多个线程是在同时执行的， CPU 资源的分配采用了时间片轮转也就是给每个线程分配一个时间片，线程在时间片内占用 CPU 执行任务。当线程使用完时间片后，就会处于就绪状态并让出 CPU 让其他线程占用，这就是上下文切换。

![图片](../../img/java/并发60问/上下文切换时机.png "上下文切换时机")
...
![图片](../../img/java/并发60问/线程间通信方式.png "线程间通信方式")
...
![图片](../../img/java/并发60问/ThreadLocal线程副本.png "ThreadLocal线程副本")
...
![图片](../../img/java/并发60问/ThreadLoca存放用户上下文.png "ThreadLoca存放用户上下文")
...
![图片](../../img/java/并发60问/ThreadLoca结构图.png "ThreadLoca结构图")
...
![图片](../../img/java/并发60问/ThreadLocal内存分配.png "ThreadLocal内存分配")
...
![图片](../../img/java/并发60问/ThreadLocalMap结构示意图.png "ThreadLocalMap结构示意图")
...
![图片](../../img/java/并发60问/ThreadLocalMap解决冲突.png "ThreadLocalMap解决冲突")
...
![图片](../../img/java/并发60问/ThreadLocalMap扩容.png "ThreadLocalMap扩容")

具体代码：

![图片](../../img/java/并发60问/ThreadLocalMap扩容 resize.png "ThreadLocalMap扩容 resize")
...
![图片](../../img/java/并发60问/Java内存模型.png "Java内存模型")

本地内存是JMM的 一个抽象概念，并不真实存在。它其实涵盖了缓存、写缓冲区、寄存器以及其他的硬件和编译器优化。

![图片](../../img/java/并发60问/实际线程工作模型.png "实际线程工作模型")
...
![图片](../../img/java/并发60问/多级指令重排.png "多级指令重排")
...
![图片](../../img/java/并发60问/双重校验单例模式异常情形.png "双重校验单例模式异常情形")
...
![图片](../../img/java/并发60问/happens-before六大规则.png "happens-before六大规则")
...
![图片](../../img/java/并发60问/as-if-serial.png "as-if-serial")

A和C之间存在数据依赖关系，同时B和C之间也存在数据依赖关系。因此在最终执行的指令序列中，C不能被重排序到A和B的前面（C排到A和B的前面，程序的结果将会被改变）。但A和B之间没有数据依赖关系，编译器和处理器可以重排序A和B之间的执行顺序。

所以最终，程序可能会有两种执行顺序：

![图片](../../img/java/并发60问/as-if-serial两种执行结果.png "as-if-serial两种执行结果")
...
![图片](../../img/java/并发60问/volatile内存可见性.png "volatile内存可见性")

> volatile怎么保证有序性的呢？

重排序可以分为编译器重排序和处理器重排序，valatile保证有序性，就是通过分别限制这两种类型的重排序。

![图片](../../img/java/并发60问/volatile重排序规则表.png "volatile重排序规则表")
...
![图片](../../img/java/并发60问/volatile写插入内存屏障后生成的指令序列示意图.png "volatile写插入内存屏障后生成的指令序列示意图")

![图片](../../img/java/并发60问/volatile写插入内存屏障后生成的指令序列示意图2.png "volatile写插入内存屏障后生成的指令序列示意图2")
...
![图片](../../img/java/并发60问/monitorenter和monitorexit.png "monitorenter和monitorexit")

2. synchronized修饰同步方法时，JVM采用 `ACC_SYNCHRONIZED`标记符来实现同步，这个标识指明了该方法是一个同步方法。

   同样可以写段代码反编译看一下。

![图片](../../img/java/并发60问/synchronized修饰同步方法.png "synchronized修饰同步方法")
...
![图片](../../img/java/并发60问/Java Montior机制.png "Java Montior机制")
Java Montior机制
...
![图片](../../img/java/并发60问/Mark Word变化.png "Mark Word变化")
...
![图片](../../img/java/并发60问/锁升级方向.png "锁升级方向")
...
![图片](../../img/java/并发60问/锁升级简略过程.png "锁升级简略过程")

完整的升级过程：

![图片](../../img/java/并发60问/synchronized 锁升级过程.png "synchronized 锁升级过程")
...
![图片](../../img/java/并发60问/synchronized和ReentrantLock的区别.png "synchronized和ReentrantLock的区别")
...
![图片](../../img/java/并发60问/ReentrantLock 非公平锁加锁流程简图.png "ReentrantLock 非公平锁加锁流程简图")
...
![图片](../../img/java/并发60问/公平锁tryAcquire.png "公平锁tryAcquire")
...
![图片](../../img/java/并发60问/CAS三大问题.png "CAS三大问题")
...
![图片](../../img/java/并发60问/Java保证原子性方法.png "Java保证原子性方法")
...
![图片](../../img/java/并发60问/原子操作类.png "原子操作类")
...
![图片](../../img/java/并发60问/死锁示意图.png "死锁示意图")

那么为什么会产生死锁呢？死锁的产生必须具备以下四个条件：

![图片](../../img/java/并发60问/死锁产生必备四条件.png "死锁产生必备四条件")
...
![图片](../../img/java/并发60问/线程死锁检测.png "线程死锁检测")
...
![图片](../../img/java/并发60问/王者荣耀等待玩家确认.jfif "王者荣耀等待玩家确认")
...
![图片](../../img/java/并发60问/想结衣结衣不依,迷爱理爱理不理.png "想结衣结衣不依,迷爱理爱理不理")

我们拿代码模拟这一场景，发现CountDownLatch无能为力了，因为CountDownLatch的使用是一次性的，无法重复利用，而这里等待了两次。此时，我们用CyclicBarrier就可以实现，因为它可以重复利用。

![图片](../../img/java/并发60问/小约翰可汗选妃模拟代码.png "小约翰可汗选妃模拟代码")

运行结果：

![图片](../../img/java/并发60问/小约翰可汗选妃模拟代码运行结果.png "小约翰可汗选妃模拟代码运行结果")
...
![图片](../../img/java/并发60问/CyclicBarrier工作流程.png "CyclicBarrier工作流程")
...
![图片](../../img/java/并发60问/英雄交换猎物.jfif "英雄交换猎物")
...
![图片](../../img/java/并发60问/线程池业务示例.png "线程池业务示例")

主要代码如下：

![图片](../../img/java/并发60问/线程池应用主要代码.png "线程池应用主要代码")
...
![图片](../../img/java/并发60问/线程池工作流程-直接办理.png "线程池工作流程-直接办理")

2. 老三发现没有空闲的窗口，就在排队区排队等。

![图片](../../img/java/并发60问/线程池工作流程-排队等待.png "线程池工作流程-排队等待")

3. 老三发现没有空闲的窗口，等待区也满了，蚌埠住了，经理一看，就让休息的小姐姐赶紧回来上班，等待区号靠前的赶紧去新窗口办，老三去排队区排队。小姐姐比较辛苦，假如一段时间发现他们可以不用接着营业，经理就让她们接着休息。

![图片](../../img/java/并发60问/线程池工作流程-排队区满.png "线程池工作流程-排队区满")

4. 老三一看，六个窗口都满了，等待区也没位置了。老三急了，要闹，经理赶紧出来了，经理该怎么办呢？

![图片](../../img/java/并发60问/线程池工作流程-等待区，排队区都满.png "线程池工作流程-等待区，排队区都满")
...
![图片](../../img/java/并发60问/线程池执行流程.png "线程池执行流程")
...
![图片](../../img/java/并发60问/线程池参数.png "线程池参数")
...
![图片](../../img/java/并发60问/线程拒绝四种策略.png "线程拒绝四种策略")
...
![图片](../../img/java/并发60问/线程池常用阻塞队列.png "线程池常用阻塞队列")
...
![图片](../../img/java/并发60问/常见线程池参数配置方案.png "常见线程池参数配置方案")
...
![图片](../../img/java/并发60问/四大线程池.png "四大线程池")
...
![图片](../../img/java/并发60问/SingleThreadExecutor运行流程.png "SingleThreadExecutor运行流程")
...
![图片](../../img/java/并发60问/FixedThreadPool.png "FixedThreadPool")
...
![图片](../../img/java/并发60问/CachedThreadPool执行流程.png "CachedThreadPool执行流程")
...
![图片](../../img/java/并发60问/ScheduledThreadPool执行流程.png "ScheduledThreadPool执行流程")
...
![图片](../../img/java/并发60问/ScheduledThreadPoolExecutor执行流程.png "ScheduledThreadPoolExecutor执行流程")
...
![图片](../../img/java/并发60问/线程池异常处理.png "线程池异常处理")
...
![图片](../../img/java/并发60问/线程池状态切换图.png "线程池状态切换图")
...
![图片](../../img/java/并发60问/JDK 线程池参数设置接口.png "JDK 线程池参数设置接口")

这里主要有两个思路：

![图片](../../img/java/并发60问/动态修改线程池参数.png "动态修改线程池参数")
...
![图片](../../img/java/并发60问/线程池评估方案.png "线程池评估方案")
...
![图片](../../img/java/并发60问/线程池调优.png "线程池调优")
...
![图片](../../img/java/并发60问/线程池主要实现流程.png "线程池主要实现流程")
...
![图片](../../img/java/并发60问/自定义线程池.png "自定义线程池")
...
关于一些并发容器，可以去看看 [面渣逆袭：Java集合连环三十问  ](https://mp.weixin.qq.com/s?__biz=MzkwODE5ODM0Ng==&mid=2247488788&idx=1&sn=01875e3e45515c2d57593cb7a01d0b6b&scene=21#wechat_redirect)，里面有 `CopyOnWriteList`和 `ConcurrentHashMap`这两种线程安全容器类的问答。。
...
![图片](../../img/java/并发60问/Fork_Join分治算法.png "Fork_Join分治算法")
...
![图片](../../img/java/并发60问/工作窃取.png "工作窃取")
